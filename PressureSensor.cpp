...
int PressureSensor::getData()
{
    ...
    {
        logger_ << ERROR << "Input/output Error (can't read from sensor)" << ENDL;
        status_ = Status::Disable;
        return int(Connecting::Error);
    }
    ...
    {
        logger_ << ERROR << "Error : Input/output Error"
            << " (can't read from sensors on second step)" << ENDL;
        status_ = Status::Disable;
        return int(Connecting::Error);
    }
...
{
	if (pressureData_){
        return pressureData_.get();
    }else
    {
        logger_ << INFO << "pressure is empty" << ENDL;
        return 0.0;
    }
}
